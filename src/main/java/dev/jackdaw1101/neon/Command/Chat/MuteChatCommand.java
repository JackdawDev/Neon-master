...
public class MuteChatCommand extends NeonCommand {
    ...
    private void playNoPermissionSound(CommandSender sender) {
        ...
        if (!(boolean) plugin.getSettings().getValue("NO-PERMISSION.USE-SOUND", true)) return;
        String sound = (String) plugin.getSettings().getValue("NO-PERMISSION.SOUND", "ENTITY_VILLAGER_NO");
